...
CODESIGN_TOOLCHAIN = "//signing/toolchains:codesign_toolchain_type"
OPENSSL_TOOLCHAIN = "//signing/toolchains:openssl_toolchain_type"

JARSIGNER_TOOLCHAIN = "//signing/toolchains:jarsigner_toolchain_type"
...
_REGISTRATION = {
    ...
    "codesign": "\"@rules_signing//signing/toolchains:codesign_toolchain\"",
    "jarsigner": "\"@rules_signing//signing/toolchains:jarsigner_toolchain\"",
}
...
def _fail_missing_toolchain(tool_kind, reason, selected_tool):
    ...
    if selected_tool == "auto":
        hint = (
            ...
            "signer you need, which requires no other native signer."
        )
    fail(
...
def _toolchain_tool(ctx, toolchain_type):
    ...
    `tool` is the standard field; `codesign` is also accepted for implementations
    using codesign.bzl's provider shape. `toolchain_type` may not
...
def _jarsigner_tool_and_support_files(ctx):
    ...
    The full JDK must reach the sandbox: jarsigner dynamically loads libraries
    from elsewhere in its runtime tree, just like openssl's Windows DLLs.
    """
    tool = _toolchain_tool(ctx, JARSIGNER_TOOLCHAIN)
    if tool == None:
        return None, []
    tc = ctx.toolchains[JARSIGNER_TOOLCHAIN]
    return tool, tc.data.to_list()
...
def signing_context(
    ...
    for kind, toolchain_type in [
        ...
        if tool_file:
            ...
            if kind == "codesign":
                inputs.extend(getattr(ctx.toolchains[toolchain_type], "data", depset()).to_list())

    # Unlike the single-file signers, jarsigner needs the full JDK tree.
    ...
    if jarsigner_file == None and _needs_toolchain(
        ...
        if JARSIGNER_TOOLCHAIN in ctx.toolchains and ctx.toolchains[JARSIGNER_TOOLCHAIN] != None:
            fail(
                "rules_signing: the jarsigner toolchain is resolved but " +
                "does not expose an executable tool.",
            )
        _fail_missing_toolchain(
            "jarsigner",
            _needs_toolchain_reason(
    ...
    return {
        p + "tool": attr.string(
            ...
            doc = "Which signer to use, or \"auto\" (the default) to select " +
                  ...
                  "runs. Individual files require only the signers they " +
                  "actually select (plus cosign for forced detached signatures). " +
                  "Registered toolchains are still analyzed even when unused.",
        ),
...
def signing_toolchains(
    ...
    All of these are `mandatory = False`: an unregistered signer is allowed
    unless `signing_context` discovers an input that requires it. This includes
    jarsigner, whose dedicated type does not resolve a JDK unless a consumer
    explicitly registers an implementation.

    Optional does not mean lazy: Bazel analyzes any matching registered
    implementation before the rule runs, even if no source needs it. A rule
    that structurally cannot need a signer can omit its type entirely.
    ...
    Args:
        ...
        jarsigner: whether to declare `JARSIGNER_TOOLCHAIN`. Set to `False`
            ...
            broken by a registered implementation that fails to analyze.
            Defaults to `True`, which is what `SIGNING_TOOLCHAINS` uses.
